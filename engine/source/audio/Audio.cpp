...
std::shared_ptr<Audio> Audio::Load(const std::string &path)
{
    ...
    auto result =
        ma_decoder_init_memory(audio->m_buffer.data(), audio->m_buffer.size(), nullptr, audio->m_decoder.get());
    if (result != MA_SUCCESS)
    {
        return nullptr;
    }
    result = ma_sound_init_from_data_source(engine, audio->m_decoder.get(), 0, NULL, audio->m_sound.get());
    if (result != MA_SUCCESS)
    {
        return nullptr;
    }
